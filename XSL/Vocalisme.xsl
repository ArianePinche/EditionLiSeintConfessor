    ...
    <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:template match="/">
            ...
            <xsl:result-document href="../analyseCorpus.txt">
                ...
                <xsl:text>Nombre de mots en A inital : </xsl:text><xsl:value-of select="count(descendant::w[matches(fn:normalize(.), $regex_ainit)])"/><xsl:text>&#10;</xsl:text>
                <xsl:text>A initiale en hiatus </xsl:text><xsl:value-of select="count(descendant::w[matches(fn:normalize(.), $regex_ahiatus)])"/><xsl:text>&#10;</xsl:text>
                <xsl:for-each-group select="descendant::w[matches(fn:normalize(.), $regex_ahiatus)]" group-by=".">
                    <xsl:sort order="ascending"/>
                    <xsl:variable select="@lemma" name="lemma"/>
                    <xsl:value-of select="."/><xsl:text> (</xsl:text><xsl:value-of select="@lemma"/><xsl:text>) : </xsl:text>
                    <xsl:for-each-group select="//w[@lemma=$lemma]" group-by=".">
                        <xsl:value-of select="."/><xsl:text>, </xsl:text>
                    </xsl:for-each-group>
                    <xsl:text>&#10;</xsl:text>
                </xsl:for-each-group>
                <xsl:text>a/ai</xsl:text><xsl:text>&#10;</xsl:text>
