...
<!-- Résoudre le problème des relatifs -->
    ...
    <xsl:template match="/">
        <xsl:result-document href="../../resultats/analyseMorphologique2_pronoms.tsv">
            ...
            <xsl:call-template name="tableauFormesPers">
                <xsl:with-param name="CatGram" select="'PROper'"/>
            </xsl:call-template>
            <xsl:text>&#10;</xsl:text>
            <xsl:text>Les pronoms demonstratifs&#10;</xsl:text>
            <xsl:call-template name="tableauFormes">
                <xsl:with-param name="CatGram" select="'PROdem'"/>
            </xsl:call-template>
            <xsl:text>&#10;</xsl:text>
            <xsl:text>Les pronoms possessifs&#10;</xsl:text>
            <xsl:call-template name="tableauFormesPers">
                <xsl:with-param name="CatGram" select="'PROpos'"/>
            </xsl:call-template>
            <xsl:text>&#10;</xsl:text>
            <!--
            <xsl:text>Les pronoms relatifs&#10;</xsl:text>
            <xsl:call-template name="tableauFormes">
                        <xsl:with-param name="CatGram" select="'PROrel\|'"/>
            </xsl:call-template>
            -->
            <xsl:text>Les pronoms interrogatifs&#10;</xsl:text>
            <xsl:call-template name="tableauFormes">
                <xsl:with-param name="CatGram" select="'PROint'"/>
            </xsl:call-template>
            <xsl:text>&#10;</xsl:text>
            <xsl:text>Les pronoms indéfinis&#10;</xsl:text>
            <xsl:call-template name="tableauFormes">
                <xsl:with-param name="CatGram" select="'PROind'"/>
            </xsl:call-template>
        </xsl:result-document>
