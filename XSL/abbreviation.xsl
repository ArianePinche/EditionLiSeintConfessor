                                ...
                                <ex>
                                    <xsl:choose>
                                        ...
                                        <xsl:when test="substring($tail, 1) != ' '">
                                            <xsl:variable name="word" select="string-join(.//tei:expan//text(), '') || tokenize($tail, ' ')[1]"/>
                                            <xsl:for-each-group select="current-group()" group-by="$word">
                                                <xsl:sort select="$word"/>
                                            <xsl:value-of
                                        ...
                                        <xsl:otherwise>
                                            <xsl:for-each-group select="current-group()" group-by="string-join(.//tei:expan//text(), '')">
                                                <xsl:sort select="string-join(.//tei:expan//text(), '')"/>
                                            <xsl:value-of
